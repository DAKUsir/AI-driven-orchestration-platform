...
// ── Internal helper — seeds sheets, returns count, no req/res needed ──
async function _doSeedSheets() {
  const seeds = [
    {
    ...
    },
    {
      ...
      description: "The original 75 must-do LeetCode problems. Gold standard for quick revision.",
      problemCount: 75,
...
const getSheets = async (req, res) => {
  try {
    ...
    const count = await SheetLink.countDocuments();
    if (count === 0) {
      await _doSeedSheets();
    }
...
const seedSheets = async (req, res) => {
  try {
    const seeded = await _doSeedSheets();
